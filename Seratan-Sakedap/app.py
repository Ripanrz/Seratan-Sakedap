    ...
    try:
        if os.path.exists(filepath):
            ...
            with open(filepath, "br+") as f:
                # Menimpa file dengan data acak sebelum dihapus (Secure Erase)
...
def sapu_bersih_otomatis():
    while True:
        ...
        for nama_file in os.listdir(FOLDER_TEMP):
            ...
            # Hapus file jika usianya lebih dari 5 menit (300 detik)
...
def pdf_ke_gambar(file_pdf):
    ...
    for i in range(len(doc)):
        ...
        pix = page.get_pixmap(dpi=150) # Tambahan DPI agar gambar lebih tajam
        img_path = buat_nama_file(file_pdf.name, "png", f"_hal_{i+1}")
        pix.save(img_path)
        paths.append(img_path)
    doc.close()
    return paths # PERBAIKAN: Mengembalikan seluruh daftar path gambar, bukan cuma index 0
...
with gr.Blocks(title="Seratan Sakedap", theme=theme, css=css) as app:
        ...
        with gr.TabItem("🔄 Konversi"):
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    out_conv = gr.File(label="Hasil Konversi", file_count="multiple") # PERBAIKAN: file_count multiple agar bisa menampung banyak gambar
    ...
    # PERBAIKAN: Reset 6 buah komponen saja (3 Input, 3 Output)
    def reset_ui():
    ...
    btn_reset.click(
        fn=reset_ui, 
        inputs=None, 
        outputs=[f_pdf, f_conv, f_img, out_pdf, out_conv, out_img]
    )
